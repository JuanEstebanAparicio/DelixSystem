document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      Alerts?.success &&
        Alerts.success(data.message || "Guardado correctamente.");
      hideModal("formModal");
      loadStorage();
      reloadCategories();
    });
...
function filtrarIngredientes() {
  ...
  const cards = document.querySelectorAll(
    ".ingredient-card.card:not(.create-card)"
  );

  cards.forEach((card) => {
    const state =
      card.querySelector(".ingredient-state")?.textContent.trim() || "";
    const name =
      card.querySelector(".ingredient-name")?.textContent.toLowerCase() || "";

    const matchesState =
      stateValue === "Todos" ||
      state.toLowerCase() === stateValue.toLowerCase();
...
toggleFilters.addEventListener("click", () => {
  ...
  toggleFilters.textContent = filtersContainer.classList.contains(
    "hidden-filters"
  )
    ? "Mostrar filtros"
    : "Ocultar filtros";
